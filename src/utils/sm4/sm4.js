...
//   this.secretKey = 'asdfghjklzxcvbnm';
...
function SM4Util() {
    ...
    // CBC模式加密，自定义 iv 和 secretKey
    this.encryptData_CBC = function (plainText, secretKey, iv) {
        const sm4 = new SM4()
        const ctx = new SM4_Context()
        ctx.isPadding = true
        ctx.mode = sm4.SM4_ENCRYPT
    ...
    //解密_CBC，自定义 iv 和 secretKey
    this.decryptData_CBC = function (cipherText, secretKey, iv) {
        let sm4 = new SM4()
        let ctx = new SM4_Context()
        ctx.isPadding = true
        ctx.mode = sm4.SM4_ENCRYPT
